 ...
 * Application requires: maxDepth, maxBins, maxMemoryInMB, data input path, output directory path, featureToInclude path (optional)
 * 
 ...
 *
 */
public class Classifier_DecisionTree {
	...
	public static void main( String[] args ) throws Exception
	{
		...
		if(args.length<5)
			throw new Exception("Application requires: maxDepth, maxBins, maxMemoryInMB, dataPath, output directory path, featureToInclude path (optional)");
		...
		Set<Integer> featuresToInclude = null;
		if(args.length>=6)
			featuresToInclude = ioManager.parseFeatureNumbers(args[5]);

		//Spark Session
		SparkSession spark;
		spark = SparkSession.builder()
				//master("local[4]")
				.appName("Classifier_DecisionTree")
				.getOrCreate();
		...
		JavaRDD<LabeledPoint> parsedData = LabeledPointManager.prepareLabeledPoints(dataNoHeader, null, featuresToInclude);
		...
		scala.collection.immutable.Map<Object, Object> categoricalFeaturesInfo = scala.collection.immutable.Map$.MODULE$.<Object, Object>empty();
		...
		Strategy strategy = new Strategy
				(
						...
						org.apache.spark.mllib.tree.impurity.Impurities.fromString(impurity), 
						...
						true, 
						10);
		final DecisionTreeModel model = DecisionTree.train(trainingData.rdd(), strategy);

		long endBuildTime = System.currentTimeMillis();
		ioManager.addLine("BUILDING model: " + (endBuildTime - startTime)/1000 + " seconds", statistics);

		//write the trees
		String treeName = outputDir+"/tree.txt";
		...
		try{
			String targetModel = PathManager.getInstance().checkPathWithDefault(outputDir+"/DT_"+date, "file");
